...
def test_gradcheck_step_angles():
    ...
    psi = (torch.randn(A, dtype=torch.float64) + 1j * torch.randn(A, dtype=torch.float64)).to(
        torch.complex128
    )
    ...
    def fn(angles_):
        coins = coins_su2_from_theta(
            angles_.to(torch.float64),
            spec=CoinsSpecSU2(dtype=torch.complex128, normalize=False, check=False),
        )
