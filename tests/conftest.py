...
def make_flipflop_portmap(
    ...
    E = uu.size
    A = 2 * E  # number of oriented arcs

    # Construct oriented arcs arrays: for each edge i: (uu[i] -> vv[i]) and (vv[i] -> uu[i])
    ...
    # We'll fill segments using a running pointer per node
